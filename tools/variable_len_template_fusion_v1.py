...
# TEMPLATE EVAL
# -----------------------------
def eval_template(frags, ranges, chunk_map, jumps, max_scan):
...
def main():
    ...
    print("=" * 60)
    print(" VARIABLE LEN TEMPLATE FUSION V1")
    print("=" * 60)
    ...
    for qid in bench_ids:
        local_rng1 = random.Random(args.seed + qid + 1000)
        local_rng2 = random.Random(args.seed + qid + 2000)

        starts1, frags1 = build_variable_fragment_set(chunks[qid], T1, 128, local_rng1)
        starts2, frags2 = build_variable_fragment_set(chunks[qid], T2, 128, local_rng2)
        ...
        ranked1 = eval_template(frags1, ranges1, chunk_map, jumps, args.max_scan)
        ranked2 = eval_template(frags2, ranges2, chunk_map, jumps, args.max_scan)

        shortlist1 = [cid for cid, _, _ in ranked1[:args.top_k]]
        shortlist2 = [cid for cid, _, _ in ranked2[:args.top_k]]

        reranked1 = rerank_exact(chunks, frags1, shortlist1)
        reranked2 = rerank_exact(chunks, frags2, shortlist2)

        if len(reranked1) > 0 and reranked1[0][0] == qid:
            t1_hit1 += 1
        if len(reranked2) > 0 and reranked2[0][0] == qid:
            t2_hit1 += 1

        # fusion union + voting
        fusion_map = defaultdict(lambda: [0, 0])  # [template_votes, raw_support_sum]

        for cid, gs, rs in ranked1[:args.top_k]:
            fusion_map[cid][0] += 1
            fusion_map[cid][1] += rs

        for cid, gs, rs in ranked2[:args.top_k]:
            fusion_map[cid][0] += 1
            fusion_map[cid][1] += rs
        ...
        # exact rerank on BOTH templates, sum ranks via exact scores
        fusion_exact = []
