...
def main():
    ...
    ap.add_argument("--show-slowest", type=int, default=10)
    args = ap.parse_args()
    ...
    print("=" * 60)
    print(" RARE ANCHOR SELFCHECK 1GB V3")
    print("=" * 60)
    ...
    try:
        for qid in picked_qids:
            ...
            slow_rows.append({
                ...
                "selected": [
                    {"start": s["start"], "sa_hits": s["sa_hits"], "chunks": s["chunks"]}
                    for s in selected
                ],
            })
    ...
    slow_rows.sort(key=lambda x: x["total_time"], reverse=True)
    ...
    print(f"  median_server_time_sec  = {statistics.median(server_times):.6f}")
    print(f"  p95_server_time_sec     = {percentile(server_times, 0.95):.6f}")
    print(f"  max_total_time_sec      = {max(total_times):.6f}")
    print(f"  max_server_time_sec     = {max(server_times):.6f}")
    ...
    print("\nSLOWEST QIDS:")
    for row in slow_rows[:args.show_slowest]:
        print(
            f"  qid={row['qid']} "
            f"total_time={row['total_time']:.6f} "
            f"server_time={row['server_time']:.6f} "
            f"shortlist_size={row['shortlist_size']} "
            f"shortlist_hit={row['shortlist_hit']}"
        )
