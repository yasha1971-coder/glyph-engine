...
def embed_texts_tokenwise(
    ...
    """
    Returns list of [seq_len, dim] float32 arrays, one per text.
    We drop special tokens via attention mask and keep contextual token embeddings.
    """
    batch = tokenizer(
        texts,
        padding=True,
    ...
    for i in range(hidden.size(0)):
        mask = attn[i].bool()
        vecs = hidden[i][mask].detach().cpu().numpy().astype(np.float32)
        out.append(vecs)
    return out
...
def main():
    ...
    print("=" * 60)
    print(" LATE INTERACTION REAL FEASIBILITY V1")
    print("=" * 60)
    ...
    for row in rows[:args.limit_queries]:
        ...
        ranked_pairs = list(zip(shortlist, [float(x) for x in scores]))
