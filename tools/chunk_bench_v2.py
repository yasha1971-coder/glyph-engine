...
def build_fragment_query(chunk: bytes, frag_len: int, nfrag: int, min_gap: int, rng):
...
def rerank_shortlist(manifest_by_id, shortlist, query_frags):
    results = []
...
def main():
    ...
    available_ids = sorted(cid for cid in manifest_by_id.keys() if cid < len(chunks))
    ...
    if not eligible_ids:
        raise ValueError("no eligible chunks for fragment-query benchmark")
    ...
    print("=" * 60)
    print("  CHUNK BENCH V2 (FRAGMENT QUERY)")
    print("=" * 60)
    ...
    for qid in bench_ids:
        ...
        query_blob, query_frags = build_fragment_query(
            query_chunk, args.frag_len, args.nfrag, args.min_gap, rng
        )
        if query_blob is None:
            continue

        shortlist = shortlist_chunks(filter_obj, query_blob, args.shortlist_k)
        ...
        reranked = rerank_shortlist(manifest_by_id, shortlist, query_frags)
