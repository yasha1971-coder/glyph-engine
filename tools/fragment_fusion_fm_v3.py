...
def read_response(path):
    ...
    with open(path, "rb") as f:
        ...
        for _ in range(num_ranges):
            ...
            total_steps = struct.unpack("<Q", f.read(8))[0]
            max_steps = struct.unpack("<Q", f.read(8))[0]
            positions = [struct.unpack("<Q", f.read(8))[0] for _ in range(count)]
            out.append((positions, total_steps, max_steps))
    return out


def eval_fragment_fusion_fm_v3(
    chunks,
    chunk_starts,
    fm,
    backend_bin,
    ...
    for _ in range(trials):
        ...
        with tempfile.TemporaryDirectory() as td:
            req = os.path.join(td, "request.bin")
            res = os.path.join(td, "response.bin")
            write_request(req, ranges)

            subprocess.run(
                [backend_bin, fm_core_bin, locate_core_bin, req, res],
                check=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )

            located = read_response(res)
    ...
    print("=" * 60)
    print("  FRAGMENT FUSION FM V3 (C++ LOCATE BACKEND)")
    print("=" * 60)
...
def main():
    ...
    eval_fragment_fusion_fm_v3(
        chunks=chunks,
        chunk_starts=chunk_starts,
        fm=fm,
        backend_bin=args.backend_bin,
        fm_core_bin=args.fm_core_bin,
        locate_core_bin=args.locate_core_bin,
        trials=args.trials,
        seed=args.seed,
