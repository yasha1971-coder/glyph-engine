...
# HYBRID SHORTLIST
# ------------------------------------------------------------
def shortlist_hybrid(filter_obj, frags, top_k, max_cid):
...
def main():
    ...
    print("=" * 60)
    print(" HYBRID PROFILE V1")
    print("=" * 60)

    filt = load_filter(args.filter)
    chunks = load_chunks(args.glyph)

    # infer indexed limit from rare branch
    ...
    for qid in ids:
        ...
        shortlist = shortlist_hybrid(filt, frags, args.top_k, max_cid=len(chunks))
