...
def main():
    ...
    print("=" * 60)
    print(" VARIABLE LEN TEMPLATE FUSION V2")
    print("=" * 60)
    print(f"trials={len(bench_ids)}")
    print(f"T1={T1}")
    print(f"T2={T2}")
    print("shared_base=True")

    for qid in bench_ids:
        local_rng = random.Random(args.seed + qid + 7777)

        total_need = max(
            sum(T1) + (len(T1) - 1) * min_gap,
            sum(T2) + (len(T2) - 1) * min_gap
        )
        ...
        for cid in fusion_shortlist:
            p1, tt1 = exact_score(frags1, chunks[cid])
            p2, tt2 = exact_score(frags2, chunks[cid])
            fusion_exact.append((cid, p1 + p2, tt1 + tt2))
