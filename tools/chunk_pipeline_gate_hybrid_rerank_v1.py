...
# COARSE GATE
# ------------------------------------------------------------
def coarse_gate(query_frags, gate_obj, gate_top_m):
    sig_len = gate_obj["sig_len"]
    stride = gate_obj["stride"]

    qsig_set = set()
    for frag in query_frags:
        for sig in iter_sigs(frag, sig_len, stride):
            qsig_set.add(sig)
    ...
    for row in gate_obj["chunks"]:
        cid = row["chunk_id"]
        chunk_sigs = row["sigs"]
        # chunk_sigs is sorted list[str], but set conversion is okay for v1
        overlap = len(qsig_set.intersection(chunk_sigs))
        if overlap > 0:
...
# ------------------------------------------------------------
# HYBRID SHORTLIST (restricted to allowed cids)
# ------------------------------------------------------------
...
def main():
    ...
    for qid in ids:
        ...
        gated_ids, _gate_rows = coarse_gate(frags, gate_obj, args.gate_top_m)
        if qid in gated_ids:
            gate_hit += 1
