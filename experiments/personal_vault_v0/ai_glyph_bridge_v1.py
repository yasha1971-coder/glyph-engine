...
def evaluate(root,plan):
    ...
    for probe in probes:
        ...
        for hit in got['valid_hits']:
            ...
            object_keys.add(key)
            evidence[key].append({
                'probe':probe,'object_offset':hit['object_offset'],
                'corpus_offset':hit['corpus_offset'],'pattern_bytes':hit['pattern_bytes'],
            })
        for key in object_keys: scores[key]+=1
        probe_reports.append({
            'probe':probe,'raw_count':got['raw_count'],'valid_count':got['valid_count'],
    ...
    fully_supported=[key for key,score in ranked if score==len(probes)]
    ...
    if not ranked:
        ...
        action='found'; selected=fully_supported[0]
    else:
    ...
    for (oid,path),score in ranked[:8]:
        ranked_out.append({
            ...
            'score_fraction':score/len(probes),'fully_supported':score==len(probes),
    ...
    if action in ('ambiguous','partial'):
        clarification={
            'needed':True,
            'reason':(
                'multiple fully-supported objects remain; ask for a distinguishing fact'
                if action=='ambiguous' else
                'some probe evidence exists but no object supports the whole plan; ask whether the user remembers one exact distinctive phrase or another concrete fact'
            ),
            'candidate_paths':[x['path'] for x in ranked_out[:5]],
            'matched_probe_summary':[{'path':x['path'],'matched_probes':x['matched_probes'],'probe_count':x['probe_count']} for x in ranked_out[:5]],
        }

    return {
        'format':'GLYPH_AI_GLYPH_BRIDGE_V1',
        'human_query':plan.get('human_query',''),'plan_id':plan.get('id'),'probes':probes,
        'action':action,
        'selected_object':None if selected is None else {'object_id':selected[0],'path':selected[1]},
        'ranked_candidates':ranked_out,'probe_reports':probe_reports,'clarification':clarification,
        'contract':{
            'ai_role':'generate probes and conduct clarification',
            'glyph_role':'exact-byte evidence and object coordinates',
            'states':['found','ambiguous','partial','not_found'],
            'partial_means':'evidence exists for part of the plan but no object supports every probe',
            'no_answer_without_full_unique_evidence':True,'oracle_not_used':True,
        }
    }


def main():
    ap=argparse.ArgumentParser();ap.add_argument('root',type=Path);ap.add_argument('--plan',type=Path,required=True);ap.add_argument('--out',type=Path)
    a=ap.parse_args();plan=json.loads(a.plan.read_text());result=evaluate(a.root,plan)
