...
int main(int argc, char** argv) {
    try {
        if (argc != 5) {
            std::cerr << "usage: rlbwt_full_query_locate_summary_v1 <bwt.rlbwt> <bwt.rlbwt.rank> <locate_core_sN.bin> <query_hex>" << std::endl;
            return 2;
        }
        ...
        std::cout << "match_count: " << (r - l) << std::endl;
        std::cout << "count: " << (r - l) << std::endl;
        std::cout << "total_steps: " << total_steps << std::endl;
        std::cout << "max_steps: " << max_steps << std::endl;

        std::cout << "offsets_suppressed: true" << std::endl;
        if (!offsets.empty()) {
            std::cout << "first_offset: " << offsets.front() << std::endl;
            std::cout << "last_offset: " << offsets.back() << std::endl;
        } else {
            std::cout << "first_offset: none" << std::endl;
            std::cout << "last_offset: none" << std::endl;
        }

        return 0;
    } catch (const std::exception& e) {
        std::cerr << "rlbwt_full_query_locate_summary_v1 error: " << e.what() << std::endl;
        return 1;
    }
}
