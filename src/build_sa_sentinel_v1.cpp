...
int main(int argc, char **argv) {
    try {
        if (argc != 3) {
            std::cerr << "Usage: build_sa_sentinel_v1 <input_corpus.bin> <output_sa.bin>\n";
            return 1;
        }
        ...
        if (file_size + 1 > static_cast<uintmax_t>(std::numeric_limits<int32_t>::max())) {
            throw std::runtime_error(
                "corpus too large for int32 sentinel SA: " +
                std::to_string(file_size) +
                " bytes + sentinel (limit: " +
        ...
        for (uint8_t c : text) {
            if (c == 0) {
                throw std::runtime_error("input contains 0x00; sentinel-safe SA requires zero-free input");
            }
        }

        text.push_back(0);
        std::cout << "indexed_bytes_with_sentinel: " << text.size() << "\n";
