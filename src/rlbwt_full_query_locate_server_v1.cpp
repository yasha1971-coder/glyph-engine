...
int main(int argc, char** argv) {
    try {
        if (argc != 4) {
            std::cerr << "usage: rlbwt_full_query_locate_server_v1 <bwt.rlbwt> <bwt.rlbwt.rank> <locate_core_sN.bin>" << std::endl;
            return 2;
        }
        ...
        std::cerr << "rlbwt_full_query_locate_server_v1 error: " << e.what() << std::endl;
        return 1;
    }
}
