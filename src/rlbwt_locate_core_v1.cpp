...
int main(int argc, char** argv) {
    try {
        if (argc != 6) {
            std::cerr << "usage: rlbwt_locate_core_v1 <bwt.rlbwt> <bwt.rlbwt.rank> <locate_core_sN.bin> <l> <r>\n";
            return 2;
        }
        ...
        if (loc.sa_size != core.raw_len) throw std::runtime_error("LOC1 sa_size != raw_len");
        if (l > r || r > core.raw_len) throw std::runtime_error("bad interval");
        ...
        std::cout << "sample_step: " << loc.sample_step << "\n";
        ...
        std::cerr << "rlbwt_locate_core_v1 error: " << e.what() << "\n";
        return 1;
    }
}
