...
class MMapSA:
    def __init__(self, path):
        ...
        self.n = self.mm.size() // 4

    def get(self, i):
        return struct.unpack_from("<I", self.mm, i * 4)[0]

    def get_chunks_for_interval(self, l, r, max_range_scan):
        if l >= r:
            return set(), False
        ...
        if n == 1:
            pos = struct.unpack_from("<I", self.mm, start)[0]
            return {pos >> CHUNK_SHIFT}, False

        raw = self.mm[start:end]
        vals = struct.unpack("<" + "I" * n, raw)
        return {v >> CHUNK_SHIFT for v in vals}, False
...
def retrieve_bytes(query_bytes, server, sa_map, args):
    t0 = time.time()
    ...
    for (start, _, ent), (l, r, cnt) in zip(windows, fm_results):
        chunks, range_truncated = sa_map.get_chunks_for_interval(
            l, r, args.max_range_scan
        )
...
def main():
    ...
    sa_map = MMapSA(args.sa)
    server = FMServer(args.server_bin, args.fm, args.bwt)

    try:
        out = retrieve_bytes(query, server, sa_map, args)
