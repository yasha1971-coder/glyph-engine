...
def retrieve_bytes(query_bytes, server, chunk_map, args):
    ...
    if qlen == 0:
        return {
            ...
            "truncated": False,
            "shortlist_top": [],
            "timings": {"total_time_sec": 0.0, "server_time_sec": 0.0, "fm_calls": 0},
        }
    ...
    if not windows_pre:
        return {
            ...
            "truncated": False,
            "shortlist_top": [],
            "timings": {"total_time_sec": time.time() - t0, "server_time_sec": 0.0, "fm_calls": 0},
        }

    hex_patterns = [frag.hex() for _, frag, _ in windows_pre]
    ...
    if all(cnt == 0 for _, _, cnt in fm_results):
        return {
            ...
            "truncated": False,
            "shortlist_top": [],
            "timings": {"total_time_sec": time.time() - t0, "server_time_sec": t2 - t1, "fm_calls": len(hex_patterns)},
        }

    scored = []
    for (start, _, ent), (l, r, cnt) in zip(windows_pre, fm_results):
